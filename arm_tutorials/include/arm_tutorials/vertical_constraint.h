...
class VerticalConstraintPlanner {
  private:
    ...
    const std::string joint_theta3_ =
        "second_to_third_arm"; // theta3（同步关节）

    // 规划参数
    const std::string planner_id_ = "RRTConnect";
    const double velocity_scaling_ = 0.5;
  ...
  public:
     ...
     */
    VerticalConstraintPlanner(const std::string &main_group_name,
                              const std::string &theta3_group_name);
